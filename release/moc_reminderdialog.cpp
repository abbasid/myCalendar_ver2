/****************************************************************************
** Meta object code from reading C++ file 'reminderdialog.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.5.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../reminderdialog.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'reminderdialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.5.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_reminderDialog_t {
    QByteArrayData data[7];
    char stringdata0[99];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_reminderDialog_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_reminderDialog_t qt_meta_stringdata_reminderDialog = {
    {
QT_MOC_LITERAL(0, 0, 14), // "reminderDialog"
QT_MOC_LITERAL(1, 15, 16), // "sendRemovedIndex"
QT_MOC_LITERAL(2, 32, 0), // ""
QT_MOC_LITERAL(3, 33, 5), // "index"
QT_MOC_LITERAL(4, 39, 17), // "sendRemindMeLater"
QT_MOC_LITERAL(5, 57, 22), // "on_deleteEvent_clicked"
QT_MOC_LITERAL(6, 80, 18) // "on_addTime_clicked"

    },
    "reminderDialog\0sendRemovedIndex\0\0index\0"
    "sendRemindMeLater\0on_deleteEvent_clicked\0"
    "on_addTime_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_reminderDialog[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x06 /* Public */,
       4,    1,   37,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    0,   40,    2, 0x08 /* Private */,
       6,    0,   41,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void reminderDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        reminderDialog *_t = static_cast<reminderDialog *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->sendRemovedIndex((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->sendRemindMeLater((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->on_deleteEvent_clicked(); break;
        case 3: _t->on_addTime_clicked(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (reminderDialog::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&reminderDialog::sendRemovedIndex)) {
                *result = 0;
            }
        }
        {
            typedef void (reminderDialog::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&reminderDialog::sendRemindMeLater)) {
                *result = 1;
            }
        }
    }
}

const QMetaObject reminderDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_reminderDialog.data,
      qt_meta_data_reminderDialog,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *reminderDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *reminderDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_reminderDialog.stringdata0))
        return static_cast<void*>(const_cast< reminderDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int reminderDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void reminderDialog::sendRemovedIndex(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void reminderDialog::sendRemindMeLater(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
